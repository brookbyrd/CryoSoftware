function [spec_filtered,u, reduced_chi_squared, rsq, uniqueFlag, mRPE] = LLS_specfit_threshold(A,data,ME,threshold_spectra, flag,basis_names)
...
% Convert to absolute RFU values as first step
data = data.*10^ME;
...
if sum(act) < m
    ...
    clear u
    % ind represents the values you want to take out
    ind = find(act == 0);
    A2 = A;
    A2(:,ind) = [];
    H = A2'*A2;
...
%% Drop out and refit any time a weak signal is included.

% Subtract noise level from each channel (everything should be in ME units)
subtractedVals = spec - threshold_spectra(:,1);

% Subtract out agents LDD
subtractedAgentVals = spec(:,1) - 10.*threshold_spectra(:,2);
passRateAgents = sum(subtractedAgentVals >= 0)./size(spec,1);

% If agent is not sufficiently present, do not fit with it
if(passRateAgents <= 0 && ~ismember(1,ind))
    % Refit without the fl. agent
    [spec,u] = LLS_specfit(A(:,2:m),data,0);
    final_u(2:m) = u;
    %warning('Fit agent signal is below LDD');
else
    % Keep the original fit
    final_u = u;
end
   
% Collect FILTERED unmixed spectra
spec_filtered =[];
for i = 1:m
    spec_filtered = [spec_filtered, final_u(i)*A(:,i)];
end

% Constrain to positive values of unmixed spectra
spec_filtered(spec_filtered<0) = 0;

% Calculate the fit based off the ME-converted values
fit = sum(spec_filtered,2);

%% Solution Uniqueness test
% Check for solution uniqueness
[Xsub,idx]=licols(spec_filtered,1e-2);

uniqueBases = length(idx);
presentBases = length(find(spec_filtered(1,[1:m]) ~= 0));

if(length(idx) ~= length(find(spec_filtered(1,[1:m]) ~= 0)))
    uniqueFlag = 0;
...
%% Convert back to ME system in last step
spec_filtered = spec_filtered./10^ME;
